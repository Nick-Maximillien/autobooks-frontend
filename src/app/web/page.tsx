...
export default function ImageUpload() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const uploadPromises = images.map(async (image) => {
      ...
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_FAST_API_URL}/web-upload/`, {
          method: "POST",
          body: formData,
          headers: {
        ...
        const insightRes = await fetch(`${process.env.NEXT_PUBLIC_FAST_API_URL}/generate-insight/`, {
          method: "POST",
        ...
        if (insightRes.ok) {
          ...
          result.insight = insightData.insight || "No insight available.";
        } else {
            ...
            <div key={idx} className="analysisResult">
              {res.error ? (
                ...
                <>
                  ...
                  <p>
                    <strong>Insight:</strong> {res.insight}
                  </p>
                </>
              )}
            </div>
